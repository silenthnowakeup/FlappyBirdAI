...
base_image, background_image, pipe_images, bird_images, \
    bird_hitmask, pipe_hitmask = load_images('data/sprites/')
...
bird_index_gen = cycle([0, 1, 2, 1])  # index generator for animation
...
class FlappyBird(object):
    ...
    # Возвращает True, если птица упала на землю или столкнулась с трубой
    def check_collision(self):
        ...
        for pipe in self.pipes:
            ...
            for i in range(2):  # Убедимся, что этот блок кода выполняется для обоих труб
                if bird_rect.colliderect(pipe_boxes[i]):  # Используем colliderect для проверки столкновения
                    # Найдем область пересечения между птицей и трубой
                    ...
                    for x in range(cropped_bbox.width):
                        for y in range(cropped_bbox.height):
                            if bird_hitmask[self.bird_index][x1 + x, y1 + y] \
                                    and pipe_hitmask[i][x2 + x, y2 + y]:
                                return True
        return False
    ...
    def next_step(self, action):
        ...
        reward = 0.1
        if action == 1:
            # Изменяем скорость птицы вверх, чтобы она взлетала
            self.cur_velocity_y = self.upward_speed
            self.is_flapped = True
        else:
            # Если действие не вверх, птица падает
            self.cur_velocity_y += self.downward_speed

        # Обновляем позицию птицы по вертикали
        self.bird_y += self.cur_velocity_y
        # Обновить счет
